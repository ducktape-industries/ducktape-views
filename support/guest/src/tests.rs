...
impl App for Probe {
    ...
    fn update(&mut self, _: ()) -> Task<()> {
        self.received += 1;
        Task::none()
    }
    fn subscription(&self) -> Subscription<()> {
        Subscription::run_with(self.key, |key| {
            host::subscribe("probe", &key.to_le_bytes()).map(|_| ())
        })
    }
}
impl SnapshotApp for Probe {
    fn snapshot(&self) -> Result<Vec<u8>, String> {
        Ok(self.received.to_le_bytes().to_vec())
    }
    fn restore(bytes: &[u8]) -> Result<Self, String> {
        Ok(Self {
            key: 1,
            received: u32::from_le_bytes(bytes.try_into().map_err(|_| "invalid probe")?),
        })
    }
}
...
fn subscriptions_retain_streams_and_cancel_changed_recipes() {
    ...
    assert!(
        ...
        "subscriptions can be restored independently"
    );
    driver.app.key = 2;
...
fn task_chaining_and_abort_preserve_effect_order() {
    let task = Task::done(Some(3))
        .and_then(|value| Task::done(value + 1))
        .chain(Task::done(5));
    assert_eq!(
        futures::executor::block_on(task.into_stream().collect::<Vec<_>>()),
        vec![4, 5]
    );
